...
const {
  getSupabase, signToken, hashPassword, verifyPassword,
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'POST' && action === 'register') {
    const { username, password, display_name } = JSON.parse(event.body || '{}');
    ...
    if (error) return err('회원가입 실패: ' + error.message, 500);

    const token = signToken({ id: user.id, username: user.username, display_name: user.display_name, avatar_color: user.avatar_color });
    return ok(
